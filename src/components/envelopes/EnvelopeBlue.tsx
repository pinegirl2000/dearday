...
// 연파랑 봉투 — EnvelopeMint 기반에서 팔레트를 라이트 블루 톤으로 교체
const PALETTE = {
  flap:        '#D8E8F4',
  flapShadow:  '#90B5D4',
  body:        '#BFD7EA',
...
export default function EnvelopeBlue({ isOpen, children, width = 320 }: EnvelopeProps) {
    ...
    <div className="relative mx-auto select-none" style={{ width, height: height + 30, perspective: 1400 }} aria-live="polite">
      ...
      <div style={{ position: 'absolute', inset: 0 }}>
        <div style={{ position: 'absolute', left: 0, top: Math.round(height * 0.18), width, height: Math.round(height * 0.82),
          ...
          boxShadow: '0 14px 35px rgba(60,90,130,0.28), inset 0 1px 0 rgba(255,255,255,0.22)', zIndex: 1 }} />
        ...
        <motion.div
          style={{ position: 'absolute', left: Math.round(width * 0.08), top: Math.round(height * 0.30),
            ...
            boxShadow: '0 3px 10px rgba(60,90,130,0.22)', zIndex: 3, overflow: 'hidden',
        ...
        >
          <svg viewBox="0 0 100 100" preserveAspectRatio="none"
            style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', backfaceVisibility: 'hidden',
              filter: 'drop-shadow(0 4px 6px rgba(60,90,130,0.28))' }}>
            <defs>
              <linearGradient id="flapFrontBlue" x1="0%" y1="0%" x2="0%" y2="100%">
                <stop offset="0%" stopColor={PALETTE.flap} />
                <stop offset="100%" stopColor={PALETTE.flapShadow} />
              </linearGradient>
            </defs>
            <path d="M 0 0 L 100 0 L 56 90 Q 50 100 44 90 Z" fill="url(#flapFrontBlue)" />
          </svg>
          <svg viewBox="0 0 100 100" preserveAspectRatio="none"
            style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', backfaceVisibility: 'hidden', transform: 'rotateX(180deg)' }}>
            <defs>
              <linearGradient id="flapBackBlue" x1="0%" y1="0%" x2="0%" y2="100%">
                <stop offset="0%" stopColor={PALETTE.inner} />
                <stop offset="100%" stopColor="#E2EDF5" />
              </linearGradient>
            </defs>
            <path d="M 0 0 L 100 0 L 56 90 Q 50 100 44 90 Z" fill="url(#flapBackBlue)" />
          </svg>
          {!isOpen && (
