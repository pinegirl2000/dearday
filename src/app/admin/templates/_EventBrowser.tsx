...
interface Props {
  /** DB allowedLayouts override */
  configs?: Record<string, string[]>;
  /** event_id → ordered template_id[] */
...
export default function EventBrowser({ configs, eventOrders }: Props) {
  ...
  const previewLayoutId = selectedTpl ? (effectiveLayouts(selectedTpl, configs)[0] || selectedTpl.layout_id) : null;
  ...
  const eventMeta = EVENT_TYPES.find((e) => e.id === eventId)!;

  return (
    <div className="space-y-4">
      {/* Event 드롭다운 */}
      <div>
        <label className="block text-[11px] font-semibold text-hydrangea-700 mb-1">Event</label>
        <select
          value={eventId}
          onChange={(e) => {
      ...
      <div>
        <div className="flex justify-between items-center mb-2">
          <label className="text-[11px] font-semibold text-hydrangea-700">
          ...
          {templates.length > 1 && (
            <button
              ...
              className={`text-[11px] px-3 py-1.5 rounded-full font-semibold active:scale-95 transition ${
                reorderMode
      ...
      {/* Preview */}
      {!reorderMode && selectedTpl && previewLayoutId && (
        <div className="pt-2">
          <div className="flex items-baseline gap-2 mb-2">
            ...
            <span className="text-[10px] text-hydrangea-400">· {getLayout(previewLayoutId).name}</span>
          </div>
