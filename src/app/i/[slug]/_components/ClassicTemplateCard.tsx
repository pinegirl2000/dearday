...
export default function ClassicTemplateCard({ card, recipientName, background, rsvpSlot, guideOverlay, editable, onFieldEdit, onFieldClick, highlightedField }: Props) {
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'relative', padding: card.layout_id === 'layout-7' ? '110px 20px 60px' : '24px 20px 60px', zIndex: 1 }}>
        ...
        <FadeUp delay={0.3}>
          <div style={{ textAlign: 'center', padding: '0 16px 16px' }}>
            {(card.event_date || card.event_place || card.map_url || card.contact_phone || card.extra_info || editable) && (
              <div style={{
                ...
                {(card.event_date || editable) && (() => {
                  ...
                  const accent = tpl?.colorMain || COLORS.primary;
                ...
                {(card.event_place || card.map_url || editable) && (
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, fontSize: 13, color: tpl?.colorMain || COLORS.textDark, letterSpacing: '0.04em', marginTop: 6 }}>
                    ...
                    {card.map_url && isUrl(card.map_url) && (
                      <a href={card.map_url} target="_blank" rel="noreferrer"
                        ...
                        style={{ display: 'inline-flex', alignItems: 'center', color: tpl?.colorMain || COLORS.primary, opacity: 0.75 }}
                      >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, alignItems: 'center', fontFamily: SANS }}>
              ...
              {(card.extra_info || editable) && card.layout_id !== 'layout-7' && (
                <div style={{
        ...
        {(card.contact_name || card.contact_phone || editable) && (
